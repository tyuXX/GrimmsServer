...
public class EntityDeathEvent {
   ...
   public static void Event(org.bukkit.event.entity.EntityDeathEvent event) {
      if (event.getEntity().getKiller() != null && event.getEntity().getKiller().getType() == EntityType.PLAYER) {
         ...
         PlayerLevelHandler.getLevelHandler(event.getEntity().getKiller()).addExp((double)50.0F);
         if (event.getEntity().getType() == EntityType.PLAYER) {
            ...
            PlayerLevelHandler.getLevelHandler(event.getEntity().getKiller()).addExp((double)150.0F);
