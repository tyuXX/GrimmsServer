   ...
   public static void Event() {
      ...
      if (ticks % 24000L == 0L) {
         for(Player player : GrimmsServer.instance.getServer().getOnlinePlayers()) {
            ...
            String jobTitleId = (String)playerStats.getStat("jobTitle", String.class);
            if (jobTitleId != null && !jobTitleId.isEmpty()) {
               double multiplier = (double)1.0F + Math.pow((double)(Integer)playerStats.getStat("level", Integer.class), (double)2.0F) / (double)100.0F;
               double payCheck = Math.ceil(((JobTitle)JobTitlesBaseValues.jobTitleBaseValues.getOrDefault(jobTitleId, null)).paycheckSize() * multiplier);
               playerStats.setStat("money", (Double)playerStats.getStat("money", Double.class) + payCheck);
