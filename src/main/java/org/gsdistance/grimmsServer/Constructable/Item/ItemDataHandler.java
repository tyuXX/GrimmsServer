...
public class ItemDataHandler {
   ...
   public <T> T getItemNBTData(String key, PersistentDataType<?, T> type) {
      return (T)(this.item.getItemMeta() != null && this.item.getItemMeta().getPersistentDataContainer().has(new NamespacedKey(this.plugin, key), type) ? this.item.getItemMeta().getPersistentDataContainer().get(new NamespacedKey(this.plugin, key), type) : null);
   }
