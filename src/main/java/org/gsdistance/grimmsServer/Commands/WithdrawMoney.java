...
public class WithdrawMoney implements CommandExecutor {
   ...
   public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
      if (sender instanceof Player) {
         if (args.length == 0) {
            return false;
         } else if (Double.parseDouble(args[0]) < (double)25.0F) {
            sender.sendMessage("Smallest banknote is 25 money.");
            return false;
         } else {
            PlayerStats playerStats = PlayerStats.getPlayerStats((Player)sender);
            if ((Double)playerStats.getStat("money", Double.class) < Double.parseDouble(args[0])) {
               sender.sendMessage("Not enough money.");
               return false;
            } else {
               if (args.length > 1) {
                  double banknoteValue = Double.parseDouble(args[0]) / (double)Integer.parseInt(args[1]);
                  if (banknoteValue < (double)25.0F) {
                     sender.sendMessage("Smallest banknote is 25 money.");
                     return false;
                  }
                  ...
                  itemDataHandler.setItemLoreData(List.of("Grimmnote - " + Shared.formatNumber(banknoteValue) + " Money", "Minted by " + sender.getName(), "Minted at " + String.valueOf(LocalDateTime.now())));
                  ...
                  itemDataHandler.setItemLoreData(List.of("Grimmnote - " + Shared.formatNumber(Double.parseDouble(args[0])) + " Money", "Minted by " + sender.getName(), "Minted at " + String.valueOf(LocalDateTime.now())));
                  ((Player)sender).getInventory().addItem(new ItemStack[]{itemStack});
               }

               playerStats.setStat("money", (Double)playerStats.getStat("money", Double.class) - Double.parseDouble(args[0]));
