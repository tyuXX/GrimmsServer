...
public class DepositMoney implements CommandExecutor {
   ...
   public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
      if (sender instanceof Player) {
         ...
         Double banknoteValue = (Double)itemDataHandler.getItemNBTData("banknoteValue", PersistentDataType.DOUBLE);
         if (banknoteValue == null) {
            sender.sendMessage("Not a banknote.");
            return false;
         } else {
            PlayerStats playerStats = PlayerStats.getPlayerStats((Player)sender);
            playerStats.setStat("money", (Double)playerStats.getStat("money", Double.class) + banknoteValue * (double)itemStack.getAmount());
