package org.gsdistance.grimmsServer.Commands.MarketComand;

import org.bukkit.Material;
import org.bukkit.command.CommandSender;
import org.bukkit.entity.Player;
import org.bukkit.inventory.ItemStack;
import org.gsdistance.grimmsServer.Shared;
import org.gsdistance.grimmsServer.Constructable.Market;

public class Sell {
   public Sell() {
   }

   public static boolean SubCommand(CommandSender sender, String[] args) {
      if (sender instanceof Player) {
         Market market = Market.getMarket();
         ItemStack iS = ((Player)sender).getInventory().getItemInMainHand();
         if (iS.getType() == Material.AIR) {
            sender.sendMessage("You are not holding anything.");
            return false;
         } else {
            double sold = market.sell(iS, (Player)sender);
            market.saveMarket();
            int var10001 = iS.getAmount();
            sender.sendMessage("You sold " + var10001 + " many of " + String.valueOf(iS.getType().getKey()) + " for " + Shared.formatNumber(Math.max((double)0.25F, (double)Math.round(sold))));
            return true;
         }
      } else {
         sender.sendMessage("This command can only be run by a player.");
         return false;
      }
   }
}
