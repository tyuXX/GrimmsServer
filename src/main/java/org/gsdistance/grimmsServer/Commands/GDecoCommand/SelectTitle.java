...
public class SelectTitle {
   ...
   public static boolean subCommand(Player player, String title) {
      ...
      if (title.isEmpty()) {
         ...
         player.sendMessage(String.valueOf(ChatColor.GREEN) + "Successfully cleared deco title.");
         return true;
      } else if (playerTitles.hasTitle(title)) {
         playerMetadata.decoTitle = title;
         playerMetadata.saveToPDS();
         player.sendMessage(String.valueOf(ChatColor.GREEN) + "Successfully set deco title.");
         return true;
      } else {
         player.sendMessage(String.valueOf(ChatColor.RED) + "Failed to set deco title.");
         return false;
      }
   }
}
