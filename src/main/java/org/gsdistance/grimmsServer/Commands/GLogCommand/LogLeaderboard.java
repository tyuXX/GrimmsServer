...
public class LogLeaderboard {
   ...
   public static boolean subCommand(Player player) {
      ...
      for(String stat : leaderBoard.leaderboard.keySet()) {
         ...
         player.sendMessage("|Leader of stat " + var10001 + " is " + (String)((Data)leaderBoard.leaderboard.get(stat)).key() + " with " + String.valueOf(((Data)leaderBoard.leaderboard.get(stat)).value()));
      }
