   ...
   public static boolean subCommand(Player player) {
      if (!player.hasPermission("grimmsserver.log.self")) {
         ...
         for(String title : playerTitles.getTitles()) {
            titlesList.add("|" + title + ": " + (String)PlayerTitles.titles.get(title));
         }

         GeneralChatHandler.sendArray(player, (String[])titlesList.toArray(new String[0]));
         return true;
      }
   }
}
