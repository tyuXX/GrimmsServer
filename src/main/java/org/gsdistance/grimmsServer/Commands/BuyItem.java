...
public class BuyItem implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
        if (sender instanceof Player) {
            ...
            if(Material.matchMaterial(args[0].toLowerCase()) == null){
                sender.sendMessage("Invalid item id:" + args[0]);
                return false;
            }
            Market market = Market.getMarket();
            if(market.items.get(args[0]) >= Integer.parseInt(args[1])){
                sender.sendMessage("Not enough stock of " + args[0] + " in the market.");
                return false;
            }
            double bought = market.buy(Material.matchMaterial(args[0].toLowerCase()), Integer.parseInt(args[1]), (Player) sender);
            market.saveMarket();
