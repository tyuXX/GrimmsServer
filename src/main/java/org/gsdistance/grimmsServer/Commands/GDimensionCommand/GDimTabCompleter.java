...
public class GDimTabCompleter implements TabCompleter {
   ...
   public java.util.List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
      if (args.length == 1) {
         return (java.util.List)Stream.of("create", "delete", "tp", "list", "info").filter((sub) -> sub.toLowerCase().startsWith(args[0].toLowerCase())).collect(Collectors.toList());
         ...
         switch (args[0].toLowerCase()) {
            ...
            case "info":
            case "tp":
               var10000 = (java.util.List)Bukkit.getWorlds().stream().map(WorldInfo::getName).filter((name) -> name.toLowerCase().startsWith(args[1].toLowerCase())).collect(Collectors.toList());
               break;
            case "delete":
         ...
         return (java.util.List)Stream.of("NORMAL", "NETHER", "THE_END").filter((type) -> type.toLowerCase().startsWith(args[2].toLowerCase())).collect(Collectors.toList());
      } else if (args.length == 4 && args[0].equalsIgnoreCase("create")) {
         return (java.util.List)Stream.of("true", "false").filter((option) -> option.toLowerCase().startsWith(args[3].toLowerCase())).collect(Collectors.toList());
      } else if (args.length == 5 && args[0].equalsIgnoreCase("create")) {
         return (java.util.List)Stream.of("normal", "flat", "amplified", "large_biomes").filter((env) -> env.toUpperCase().startsWith(args[4].toUpperCase())).collect(Collectors.toList());
